...
async function onSubmit() {
    try {
        ...
        const response = await axios.post(`${import.meta.env.VITE__APP_URL}/api/register`, formData, {
            headers: {
...
<template>
    ...
    <div class="pt-28 lg:pb-60 xl:pb-20 px-16 flex flex-col items-center justify-center">
        ...
        <div class="border border-slate-200 py-2 px-2 lg:w-full xl:w-1/2">
            <form action="#" @submit.prevent="onSubmit">
                ...
                <div class="pt-4">
                    ...
                    <span class="text-red-500 py-0 my-0 text-xs">{{ validationError?.name[0] }}</span>
                    <input v-model="state.name" id="name" type="text" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
                ...
                <div class="pt-4">
                    ...
                    <span class="text-red-500 py-0 my-0 text-xs">{{ validationError?.password[0] }}</span>
                    <input v-model="state.password" id="no_wa" type="password" class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline">
                </div>
        ...
        <div class="pt-10">
            <p>
                Login <router-link to="login" class="underline font-bold">Disini</router-link>
            </p>
        </div>
    </div>
